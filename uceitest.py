...
def raster_paths_ydir(poly, spacing, overrun=0.0):  #used for crosshatch - overrun is asymmetrical in default function because of rotation
    minx, miny, maxx, maxy = poly.bounds
    paths = []
    direction = 1
...
height_label.grid(row=11, column=0, pady=(20, 0))
heightEntry = tk.Entry(control_frame, width=15)
heightEntry.grid(row=12, column=0)
# tk.Button(control_frame, text="Move servo", command=move_servo).grid(row=10, column=0)
###### SERVO SELECTION #######

generate_button = tk.Button(control_frame, text="Generate!", command=finish)
generate_button.grid(row=13, column=0, pady=(25, 0))
